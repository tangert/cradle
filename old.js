...
use wikilink syntax as shorthand

user flow  {
  branch 1  -> step 1 -> step 2 -> step 3 {
    branch 1.3 -> go back to start -> start
  } 
  branch 2 -> step 1 -> step 2 -> step 3
}

sign up flow  {
  if ur a new member -> enter in credentials {
    if everything works -> hooray
    if theres an error {
      fix error until it works -> hooray   
    } 
  }
}

start {
  first go to [[branch 1]], then [[step 2]], then [[step 3]] {
    branch 1.3 -> go back to start -> start
  } 
  branch 2 -> step 1 -> step 2 -> step 3
}

maybe think about titles and descriptions?

// this is short hand for:
start {
  first go to branch 1 -> branch 1 -> then go to step 2 -> step 2 -> then step 3 -> step 3 {
    branch 1.3 -> go back to start -> start
  } 
  branch 2 -> step 1 -> step 2 -> step 3
}
